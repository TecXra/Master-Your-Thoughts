<template>
	<div>
		<b-tabs content-class="mt-3" fill>
			<b-tab active @click="updateSearchText('HD Cool Wallpapers')">
				<template #title>
				...
				<div class="row" v-else>
					<div class="col-sm-12 col-md-4 col-lg-3 col-xl-3 text-center" v-for="photo in photos" :key="photo.id">
						<img class="image-list cursor-pointer" height="200" width="200" :src="photo.urls.thumb" alt="img" @click="applyPhoto(photo)" />
					</div>
				</div>
			</b-tab>

			<b-tab @click="updateSearchText('HD Nature Wallpapers')">
				<template #title>
					<p class="text-primary mb-0 font-weight-bold">
						Nature
					</p>
	...
	export default {
		...
		data () {
			return {
				isLoading: true,
				photos: [],
				search: 'explore HD Wallpapers Cool Wallpapers',
				photo_url: null,
				download_url: null,
				rows: 100,
				perPage: 1,
		...
		methods: {
			...
			// await this.$axios.$get(`https://api.unsplash.com/search/collections?page=${this.currentPage}&per_page=12&query=${this.search}&client_id=${appid}`)
				await this.$axios.$get(`https://api.unsplash.com/search/photos?page=${this.currentPage}&per_page=12&order_by=relevant&orientation=landscape&query=${this.search}&client_id=${appid}`)
				.then(response => {
